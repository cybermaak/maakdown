...
| P12.3 | Done | Windows Markdown file association | Full stack | P12.1 | HKCU ProgId + RegisteredApplications add Maakdown to Open With/Default Apps; Settings row launches the system chooser (no silent default on Win10+) | real Windows session verified five `REG_NONE` OpenWith entries, ProgId command/icon, per-user capabilities, unchanged existing default, argv open to the requested tab, Windows build, Go tests, frontend checks, and UAT-09 (5 scenarios) |
