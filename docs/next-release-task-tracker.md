...
| P17 | In Progress | Theme, accessibility, and release acceptance | macOS acceptance passed; Windows/Linux release validation blocks release; initial cross-OS CI/UAT passed on `main` |
| P18 | Done | Table reading tools | macOS unit, UAT, build, and benchmark validation passed; Windows/Linux release validation pending |
...
| ID | Status | Task | Owner | Depends On | Exit Criteria | Mac Validation | Windows Validation | Linux Validation |
|---|---|---|---|---|---|---|---|---|
| P18.1 | Done | Define sanitized table projection model | Frontend Core | P1/P13 | table parser projects headers, rows, text values, alignment, row/column counts, and no-header/large-table capability flags from sanitized block HTML without changing the document model | Passed: `npm test -- --run src/core/tables/tableProjection.test.ts`; full `npm test` | Pending: CI unit tests after P18 push | Pending: CI unit tests after P18 push |
| P18.2 | Done | Add table layout settings | Frontend/Backend | P18.1/P10 | settings persist a table-measure constraint option and a simple column-sizing mode with backward-compatible config migration | Passed: `go test ./internal/config`, `go test ./...`, `npm run check`, and UAT-12 settings path | Pending: Windows config/session UAT after P18 push | Pending: Linux config/session UAT after P18 push |
| P18.3 | Done | Render constrained auto-wrapping tables | Frontend | P18.1/P18.2 | constrained tables respect the active reader measure, use deterministic auto column widths, wrap cell text, and keep unconstrained behavior available | Passed: UAT-12 verifies constrained width, selected measure, and long-cell wrapping on macOS | Pending: Windows screenshot/UAT after P18 push | Pending: Linux screenshot/UAT after P18 push |
| P18.4 | Done | Add headered-table filter controls | Frontend | P18.1/P18.3 | headered tables below capability limits expose a search field that filters rendered rows only; no-header and over-limit tables suppress controls | Passed: UAT-12 verifies filtering, row counts, no-header suppression, and over-limit suppression | Pending: Windows UAT after P18 push | Pending: Linux UAT after P18 push |
| P18.5 | Done | Add stable column sort controls | Frontend | P18.1/P18.3 | headered tables below capability limits sort by selected column with asc/desc/source-order cycling, including numeric/date/text comparisons, without editing Markdown | Passed: table projection unit tests and UAT-12 verify numeric/date/text stable sort and source-order restore | Pending: Windows UAT after P18 push | Pending: Linux UAT after P18 push |
| P18.6 | Done | Preserve table interaction state during reader session | Frontend | P18.4/P18.5 | table filter/sort state survives virtualizer remounts for the current document/tab session but is not written into source Markdown | Passed: UAT-12 scrolls away/back and verifies filter state survives remount for the current session | Pending: Windows UAT/manual pass after P18 push | Pending: Linux UAT/manual pass after P18 push |
| P18.7 | Done | Add table reader tests and benchmarks | Frontend QA/Perf | P18.1-P18.6 | unit tests, UAT cases, and a bounded benchmark cover table parsing, layout, sort/filter limits, and no material regression to reader performance | Passed: `npm test`, focused UAT-12, full `npm run uat` (31 tests), `npm run benchmark`, `npm run benchmark:workspace`, `npm run build`, and `scripts/release-check.sh` on macOS | Pending: GitHub CI Windows UAT after P18 push | Pending: GitHub CI Linux UAT after P18 push |
