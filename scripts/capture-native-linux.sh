...
while IFS=$'\t' read -r -a fields; do
  ...
  for theme in light dark; do
    ...
    XDG_CONFIG_HOME="$config_root" "$app" "${fixtures[@]}" >"$output_dir/linux-$slug-$theme.log" 2>&1 &
    app_pid=$!
    trap 'kill "$app_pid" 2>/dev/null || true' EXIT

    # WebKitGTK enhancement work is asynchronous. The fixed wait makes the
    # resulting artifact suitable for human spot checks.
    sleep 10
    import -window root "$output_dir/linux-$slug-$theme.png"
